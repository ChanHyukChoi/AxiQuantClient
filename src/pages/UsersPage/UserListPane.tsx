...
export const UserListPane = ({
  ...
  <>
    <style>{`
      ...
      .user-list-scroll::-webkit-scrollbar-thumb { background: var(--color-border); border-radius: 2px; }
      .user-list-search::placeholder { color: var(--color-text-dim); }
    `}</style>
      ...
      >
        ...
        >
          ...
          <input
            ...
            className="user-list-search flex-1 text-[12px] outline-none min-w-0"
            style={{ background: 'transparent', color: 'var(--color-text)' }}
          />
        </div>
      </div>

      <div className="flex-1 overflow-y-auto user-list-scroll">
        {loading ? (
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge variant={user.active ? 'on' : 'off'}>{user.active ? '활성' : '비활성'}</Badge>
                </div>
                <p className="text-[11px] mt-0.5 font-mono truncate" style={{ color: 'var(--color-text-subtle)' }}>
                  {user.loginId}
                </p>
              </div>
            )
