...
interface HolidayListPaneProps {
  items: HolidayInfo[]
...
export const HolidayListPane = ({
  ...
  >
    ...
    >
      <SearchField value={searchQuery} placeholder="휴일 검색..." onChange={onSearch} />
    </div>
    ...
    <div className="flex-1 overflow-y-auto app-scrollbar">
      {loading ? (
        ...
        <p className="text-[14px] text-center py-8" style={{ color: 'var(--color-text-subtle)' }}>
          {searchQuery.trim() ? '검색 결과가 없습니다.' : '등록된 휴일이 없습니다.'}
        </p>
      ) : (
        items.map((item) => {
